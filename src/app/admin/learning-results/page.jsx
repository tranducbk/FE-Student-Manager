    ...
    if (token) {
      try {
        const res = await axios.get(
          `https://be-student-manager.onrender.com/commander/learningResults?semester=${semester}`,
          {
            headers: {
              token: `Bearer ${token}`,
            },
    ...
    if (token) {
      try {
        const response = await axios.get(
          `https://be-student-manager.onrender.com/commander/learningResult/pdf?semester=${semester}`,
          {
            headers: {
              token: `Bearer ${token}`,
            },
        ...
        <div className="w-full pt-8 pb-5 pl-5 pr-6 mb-5">
          <div className="bg-white rounded-lg w-full">
            <div className="font-bold p-5 flex justify-between">
              <div>KẾT QUẢ HỌC TẬP HỌC VIÊN</div>
              <button
